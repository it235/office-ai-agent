...
Public Class MemoryRepository
    ...
    ''' </summary>
    Public Shared Sub InsertAtomicMemory(content As String, Optional tags As String = Nothing, Optional sessionId As String = Nothing, Optional appType As String = Nothing)
        ...
        Using conn As New SQLiteConnection(OfficeAiDatabase.GetConnectionString())
            ...
            Using cmd As New SQLiteCommand(
                "INSERT INTO atomic_memory (timestamp, content, tags, session_id, app_type) VALUES (@ts, @content, @tags, @sid, @app)", conn)
    ...
    Public Shared Function ListAtomicMemories(Optional limit As Integer = 100, Optional offset As Integer = 0, Optional appType As String = Nothing) As List(Of AtomicMemoryRecord)
        ...
        Dim sql = "SELECT id, timestamp, content, tags, session_id, create_time FROM atomic_memory WHERE 1=1"
    ...
    ''' <summary>
    ''' 按关键词与时间范围检索原子记忆（初版简单 LIKE）
    ''' </summary>
    Public Shared Function GetRelevantMemories(query As String, topN As Integer, Optional startTime As DateTime? = Nothing, Optional endTime As DateTime? = Nothing) As List(Of AtomicMemoryRecord)
        OfficeAiDatabase.EnsureInitialized()
        Dim list As New List(Of AtomicMemoryRecord)()
        Dim sql = "SELECT id, timestamp, content, tags, session_id, create_time FROM atomic_memory WHERE 1=1"
        ...
        If Not String.IsNullOrWhiteSpace(query) Then
            sql &= " AND (content LIKE @q OR tags LIKE @q)"
        End If
